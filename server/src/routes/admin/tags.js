...
router.get('/', adminAuth, async (req, res) => {
  const tags = (await db.all('SELECT id, name, category, usage_count, embedding, created_at FROM tag_library ORDER BY usage_count DESC'))
...
router.post('/init', adminAuth, async (req, res) => {
  try {
    const rows = await db.all('SELECT tags FROM cards');
    ...
    const existingTags = new Set((await db.all('SELECT name FROM tag_library')).map(r => r.name));
    ...
    for (let i = 0; i < newTags.length; i += batchSize) {
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          try {
            await db.run('INSERT IGNORE INTO tag_library (name, embedding) VALUES (?, ?)', batch[j], JSON.stringify(embeddings[j]));
            added++;
          } catch { failed++; }
        }
      } catch (e) {
        ...
        for (const t of batch) {
          await db.run('INSERT IGNORE INTO tag_library (name) VALUES (?)', t);
          added++;
        }
      }
...
router.delete('/:id', adminAuth, async (req, res) => {
  ...
  const tag = await db.get('SELECT id, name FROM tag_library WHERE id = ?', id);
  if (!tag) return res.status(404).json({ error: '标签不存在' });
  await db.run('DELETE FROM tag_library WHERE id = ?', id);
...
router.post('/merge', adminAuth, async (req, res) => {
  try {
    ...
    for (const id of sourceIds) {
      const tag = await db.get('SELECT name, usage_count FROM tag_library WHERE id = ?', id);
      if (tag) { sourceNames.push(tag.name); totalUsage += tag.usage_count; }
    }

    const cleanTarget = targetName.replace(/^#/, '').trim();
    const existingTarget = await db.get('SELECT id FROM tag_library WHERE name = ?', cleanTarget);

    if (existingTarget) {
      await db.run('UPDATE tag_library SET usage_count = usage_count + ? WHERE id = ?', totalUsage, existingTarget.id);
    } else {
      try {
        const emb = await getEmbedding(cleanTarget);
        await db.run('INSERT INTO tag_library (name, usage_count, embedding) VALUES (?, ?, ?)', cleanTarget, totalUsage, JSON.stringify(emb));
      } catch {
        await db.run('INSERT INTO tag_library (name, usage_count) VALUES (?, ?)', cleanTarget, totalUsage);
      }
    }

    for (const id of sourceIds) {
      const tag = await db.get('SELECT name FROM tag_library WHERE id = ?', id);
      if (tag && tag.name !== cleanTarget) await db.run('DELETE FROM tag_library WHERE id = ?', id);
    }

    for (const srcName of sourceNames) {
      const cards = await db.all("SELECT id, tags FROM cards WHERE tags LIKE ?", `%${srcName}%`);
      for (const card of cards) {
        try {
          const tags = JSON.parse(card.tags);
          const updated = tags.map(t => t.replace(/^#/, '') === srcName ? '#' + cleanTarget : t);
          const deduped = [...new Set(updated)];
          await db.run('UPDATE cards SET tags = ? WHERE id = ?', JSON.stringify(deduped), card.id);
        } catch {}
      }
    }
